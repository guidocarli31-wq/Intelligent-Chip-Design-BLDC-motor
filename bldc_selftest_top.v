...
//   * led[5:0] "breathe" (brightness follows each phase's sine duty), the
//     three phases 120 deg apart, cycling at ~2 Hz. That breathing IS the
//     hardware sine/SVPWM injection working.
//
...
module bldc_selftest_top #(
    ...
    input  wire       rst_btn_n,   // reset button, active-low (tie high if unused)
    output wire [5:0] pwm_out,     // to scope header / gate-driver inputs
    output wire [5:0] led          // on-board LEDs (see note on polarity below)
);
    wire clk = clk_in;

    //------------------------------------------------------------------
    // Power-on reset: hold reset for 65536 cycles, then follow the button
    //------------------------------------------------------------------
    reg [15:0] por = 16'd0;
    reg        rst_n_r = 1'b0;
    ...
    //------------------------------------------------------------------
    // LEDs. Tang Primer 20K dock LEDs are ACTIVE-LOW (lit = 0), so invert.
    // If yours are active-high, use  assign led = pwm_out;
    //------------------------------------------------------------------
    assign led = ~pwm_out;
